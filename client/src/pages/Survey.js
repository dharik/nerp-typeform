import { Box, Button, Container, Spacer, Tab, TabList, TabPanel, TabPanels, Tabs, Text, useToast, VStack } from "@chakra-ui/react"
...
export default function Survey() {
    ...
    const loadSurveyQuery = useQuery('survey', async () => {
    ...
    const submitSurveyMutation = useMutation('submitSurvey', async (responseData) => {
    ...
    return <Container maxW={"container.xl"} padding="10">
        {submitSurveyMutation.isSuccess ?
            ...
            :
            <Question {...loadSurveyQuery.data.questions[0]} onSetResponse={(selectedChoice) => {
                ...
                if (loadSurveyQuery.data.canManage) {
                    toaster({
                        status: "info",
                        description: "Responses via owner's link aren't saved. Try a different share link if you want to capture responses",
                        duration: 3000,
                        variant: 'left-accent'
                    })
                } else {
